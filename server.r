...
if(runif(1,0,1)<0.6) {
  if(runif(1,0,1)<0.5) sd_bound[1]<--5 else sd_bound[2]<-5
}
...
normal_plot <- function(mean, sd, lb, ub, sd_bound) {
  ...
  # mean_txt <- paste("Mean: ", mean) - Changed the text above the graph a bit
  sd_text <- paste("and S.D: ", sd)
  if(sd_bound[1]==-5) {
    result <- paste(rv_text, "\n", mean_txt, "\n", sd_text, "\n", "What is P(X <",ub,")?")  
  }
  else {
    if(sd_bound[2]==5) result <- paste(rv_text, "\n", mean_txt, "\n", sd_text, "\n", "What is P(X >",lb,")?")
    else result <- paste(rv_text, "\n", mean_txt, "\n", sd_text, "\n", "What is P(",lb,"< X <",ub,")?")
  }
...
shinyServer(function(input, output, session){
  ...
  observeEvent(input$action, {
    ...
    if(runif(1,0,1)<0.6) {
      if(runif(1,0,1)<0.5) v$sd_bound[1]<--5 else v$sd_bound[2]<-5
    }
  ...
  output$NormalPlot <- renderPlot(normal_plot(v$mean, v$sd, v$bounds[1], v$bounds[2], v$sd_bound))
